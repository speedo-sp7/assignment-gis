...
SELECT s.osm_id, s.amenity, s.operator, ST_asGeoJSON( ST_transform( s.way, 4326 ) )::JSON AS center_json, t.importance AS importance
...
INNER JOIN
  (WITH stations AS
     (SELECT p.osm_id,
      ...
      WHERE (((p.amenity = '')
              OR (p.amenity = 'fuel')
              OR (p.amenity = 'charging_station'))
             AND ( ST_contains( ST_transform( ST_MakeEnvelope( 17.028250694274906, 48.18789181411507, 17.075972557067875, 48.210375307610235, 4326 ), 3857 ), p.way ) ))),
        roads AS
     (SELECT l.osm_id,
             l.way,
             l.highway
      ...
      WHERE ((l.highway = 'primary'
             ...
             AND ( ST_intersects( ST_transform( ST_MakeEnvelope( 17.028250694274906, 48.18789181411507, 17.075972557067875, 48.210375307610235, 4326 ), 3857 ), l.way ) )))
   SELECT s.osm_id AS osm_id, max( CASE
   ...
   GROUP BY s.osm_id) AS t
ON (s.osm_id = t.osm_id)
